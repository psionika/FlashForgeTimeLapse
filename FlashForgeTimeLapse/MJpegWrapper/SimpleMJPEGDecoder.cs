...
namespace FlashForgeTimeLapse.MJpegWrapper
{
    public static class SimpleMJPEGDecoder
    {
        ...
        /// <param name="url">url of the http stream (only basic authorization is implemented)</param>
        /// <param name="login">optional login</param>
        /// <param name="password">optional password (only basic authorization is implemented)</param>
        ...
        public async static Task StartAsync(
            Action<BitmapSource> action,
            string url,
            string login = null,
            string password = null,
            CancellationToken? token = null,
            int chunkMaxSize = 1024,
            int frameBufferSize = 1024 * 1024)
        {
            ...
            do
            {
                ...
                if (previous == picMarker && current == picEnd)
                {
                    ...
                    using (var s = new MemoryStream(frameBuffer, 0, frameIdx))
                    {
                        try
                        {
                            JpegBitmapDecoder decoder = new JpegBitmapDecoder(s, BitmapCreateOptions.PreservePixelFormat, BitmapCacheOption.Default);
